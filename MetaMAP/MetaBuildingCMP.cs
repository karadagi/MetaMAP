...
public class MetaBuildingCMP : GH_Component
{
    ...
    {
        return Core.OverpassQueryBuilder.BuildBuildingQuery(lat, lon, radius);
    }
    ...
    {
        return Core.BuildingHeightParser.GetHeightMeters(element.Tags);
    }
    ...
    private Brep CreateBuildingBrep(Polyline polyline, double height)
    {
        try
        {
            ...
            if (!TestHooks.ForceExtrusionFailure)
            {
    ...
    {
        return Core.BuildingHeightParser.GetBuildingType(element.Tags);
    }
